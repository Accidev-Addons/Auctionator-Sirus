...
function AuctionatorSaleItemMixin:OnLoad()
  ...
  self.Deposit:SetPoint("TOPLEFT", self.Duration, "TOPRIGHT", -10, 0)
...
function AuctionatorSaleItemMixin:UnlockItem()
  if self.itemInfo ~= nil then
    ...
    if self.itemInfo.count > 0 and self.itemInfo.location:IsValid() then
...
function AuctionatorSaleItemMixin:OnUpdate(elapsed)
  ...
  elseif not self.itemInfo.location:IsValid() then
...
function AuctionatorSaleItemMixin:DoSearch(itemInfo, ...)
  FrameUtil.RegisterFrameForCustomEvents(self, SALE_ITEM_EVENTS)
  ...
  if IsEquipment(itemInfo) and not Auctionator.Config.Get(Auctionator.Config.Options.SELLING_GEAR_USE_ILVL) then
    ...
    self.expectedItemKey = {itemID = itemInfo.itemKey.itemID, itemLevel = 0, itemSuffix = 0}
...
function AuctionatorSaleItemMixin:OnEvent(eventName, ...)
  if eventName == "COMMODITY_SEARCH_RESULTS_UPDATED" then
    ...
    FrameUtil.UnregisterFrameForCustomEvents(self, SALE_ITEM_EVENTS)
